...
class DeliveryService {
    constructor(supabase, redis // Simplified type to avoid Redis type issues
        ...
        this.USE_REDIS = false; // 🔥 TEMPORARY: Disable Redis for debugging
        console.log('[DeliveryService] Initialized - USE_REDIS:', this.USE_REDIS);
    }
    matchesUserProfile(campaign, userProfile, debug = false) {
        ...
        if (campaign.target_users && campaign.target_users.length > 0) {
            if (!userProfile.user_id || !campaign.target_users.includes(userProfile.user_id)) {
                if (debug)
                    console.log(`  ❌ Campaign "${campaign.title}" - specific user targeting mismatch`);
                return false;
            }
        }
        // Gender matching
        if (campaign.target_gender && campaign.target_gender !== userProfile.gender) {
            if (debug)
                console.log(`  ❌ Campaign "${campaign.title}" - gender="${campaign.target_gender}" but user gender="${userProfile.gender}"`);
            return false;
        }
        // User type matching (bachelier / etudiant / parent)
        if (campaign.target_user_type && campaign.target_user_type !== userProfile.user_type) {
            if (debug)
                console.log(`  ❌ Campaign "${campaign.title}" - userType="${campaign.target_user_type}" but user type="${userProfile.user_type}"`);
            return false;
        }
        // Age matching
        if (campaign.min_age && userProfile.age && userProfile.age < campaign.min_age) {
            if (debug)
                console.log(`  ❌ Campaign "${campaign.title}" - minAge=${campaign.min_age} but user age=${userProfile.age}`);
            return false;
        }
        // Location matching
        if (campaign.location && campaign.location !== userProfile.location) {
            if (debug)
                console.log(`  ❌ Campaign "${campaign.title}" - location="${campaign.location}" but user location="${userProfile.location}"`);
            return false;
        }
        if (debug)
            console.log(`  ✅ Campaign "${campaign.title}" - MATCHES all targeting criteria`);
        return true;
    }
    async getCarouselAds(userProfile = {}) {
        ...
        if (this.USE_REDIS) {
            try {
                ...
                if (this.redis && this.redis.isOpen) {
                    ...
                    const cacheTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Redis timeout')), 2000));
                ...
                console.warn('[getCarouselAds] Redis cache read failed:', error.message);
            }
        }
        else {
        ...
        console.log('[getCarouselAds] STEP 4 - Got campaigns, count:', campaigns?.length);
        if (campaigns && campaigns.length > 0) {
            console.log('[getCarouselAds] First campaign structure:', JSON.stringify(campaigns[0], null, 2));
        }
        // Manual JOIN and filter by user profile
        console.log(`[getCarouselAds] STEP 5 - Filtering ${campaigns?.length} campaigns for user:`, Object.keys(userProfile).length > 0 ? userProfile : '(no targeting applied)');
        const filteredCampaigns = campaigns.filter((campaign) => this.matchesUserProfile(campaign, userProfile, true));
        ...
        if (this.USE_REDIS) {
            try {
                ...
                console.warn('[getCarouselAds] Redis cache write failed:', error.message);
            }
        }
    ...
    async getShortsAds(userProfile = {}) {
        ...
        if (this.USE_REDIS) {
            try {
                ...
                console.warn('[getShortsAds] Redis cache read failed:', error.message);
            }
        }
        else {
        ...
        console.log('[getShortsAds] STEP 4 - Got campaigns, count:', campaigns?.length);
        if (campaigns && campaigns.length > 0) {
            console.log('[getShortsAds] First campaign structure:', JSON.stringify(campaigns[0], null, 2));
        }
        // Manual JOIN and filter by user profile
        console.log(`[getShortsAds] STEP 5 - Filtering ${campaigns?.length} campaigns for user:`, Object.keys(userProfile).length > 0 ? userProfile : '(no targeting applied)');
        const filteredCampaigns = campaigns.filter((campaign) => this.matchesUserProfile(campaign, userProfile, true));
        ...
        if (this.USE_REDIS) {
            try {
                ...
                console.warn('[getShortsAds] Redis cache write failed:', error.message);
            }
        }
